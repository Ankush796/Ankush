...
log_group = os.getenv('LOG_GROUP')  # ab LOG_GROUP ka use ho raha hai

if not all([api_id, api_hash, bot_token, log_group]):
    logger.error("API_ID, API_HASH, BOT_TOKEN, and LOG_GROUP environment variables must be set!")
    sys.exit(1)
...
async def send_telegram_log(message: str):
    try:
        await app_pyro.send_message(log_group, message)
